...
export function resolveSafeRitualDescription(item: Item): string | null {
  ...
  if (typeof value !== "string" || !stripMarkup(value).trim()) return null;
...
export function sanitizeRitualDescriptionHtml(value: string): string {
  ...
  const tokens: string[] = [];
  const tokenized = withoutUnsafeBlocks.replace(/<\/?([a-z0-9]+)\b[^>]*>/giu, (tag, name: string) => {
    const normalized = name.toLowerCase();
    if (!ALLOWED_TAGS.has(normalized)) return "";
    const closing = /^<\s*\//u.test(tag);
    const safe = normalized === "br" ? "<br>" : closing ? `</${normalized}>` : `<${normalized}>`;
    const token = `${TOKEN_PREFIX}${tokens.length}__`;
    tokens.push(safe);
    return token;
  });
  let escaped = escapeHtml(tokenized);
  tokens.forEach((token, index) => { escaped = escaped.replace(`${TOKEN_PREFIX}${index}__`, token); });
  return escaped;
}
