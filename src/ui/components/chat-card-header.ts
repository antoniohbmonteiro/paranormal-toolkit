...
export type ChatCardBadgeModel = {
  label: string;
...
export function renderChatCardHeader(model: ChatCardHeaderModel): HtmlString {
  ...
  const badges = model.badges?.length
    ? `<div class="paranormal-toolkit-chat-card-header__badges">${model.badges
        .map(
          (badge) =>
            `<span class="paranormal-toolkit-chat-card-header__badge paranormal-toolkit-chat-card-header__badge--${badge.tone ?? "neutral"}">${escapeHtml(badge.label)}</span>`,
        )
        .join("")}</div>`
    : "";
  ...
  return markTrustedHtml(`
    <header class="paranormal-toolkit-chat-card-header">
      ...
      <div class="paranormal-toolkit-chat-card-header__body">
        <p class="paranormal-toolkit-chat-card-header__eyebrow">${escapeHtml(model.eyebrow)}</p>
        <h3 class="paranormal-toolkit-chat-card-header__title">${escapeHtml(model.title)}</h3>
        ${model.subtitle ? `<p class="paranormal-toolkit-chat-card-header__subtitle">${escapeHtml(model.subtitle)}</p>` : ""}
        ${badges}
      </div>
    </header>
  `);
