...
describe("renderRitualAssistedActionsPanel", () => {
  ...
  it("uses top and row separators without host surface", () => { const css=readFileSync("styles/components/ritual-assisted-actions-panel.css","utf8"); expect(css).toContain("border-top: 1px solid"); expect(css).not.toMatch(/background\s*:|box-shadow\s*:|border-left\s*:/); });
